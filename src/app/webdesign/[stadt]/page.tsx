...
export async function generateMetadata({
  ...
  // Conversion-fokussierter Title für Top-Tier (höhere CTR durch Preis + 24h-Versprechen).
  const title = isTop
    ? `Webdesigner ${region.city} · Festpreis ab 990€ · Entwurf in 24h`
    : `Webdesign ${region.city} | Fylu Webdesigner Saarland`;
  const description = isTop
    ? `Webdesigner für ${region.city}: schnelle, mobile Websites mit lokalem SEO. Festpreis ab 990€, kostenloser Entwurf in 24h, persönliche Betreuung aus Saarlouis.`
    : `Webdesign aus dem Saarland für ${region.city}. Ab 990€ mit kostenlosem Entwurf in 24 Stunden.`;
  return {
    title,
    description,
